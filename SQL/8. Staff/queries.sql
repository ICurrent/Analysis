...
SELECT s.gender, MIN(s.salary) Min_Salary, MAX(s.salary) Max_Salary, AVG(s.salary)
FROM staff s
GROUP BY 1;

/* Total salary of each department spending*/
SELECT c.company_division, GROUP_CONCAT(DISTINCT c.department SEPARATOR '\n') Department, SUM(s.salary) 'Total Salary'
FROM company_divisions c
JOIN staff s ON c.department = s.department
GROUP BY 1;
...
/*SEEMS WRONG */
SELECT SUM(tab.Total_Salary)
FROM (
        SELECT c.company_division, GROUP_CONCAT(DISTINCT c.department SEPARATOR '\n') Department, SUM(s.salary) 'Total_Salary'
    FROM company_divisions c
    JOIN staff s ON c.department = s.department
    GROUP BY 1
    )tab
